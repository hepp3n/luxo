...
use crate::{
    ...
    render::*,
    shell::WindowElement,
    state::{take_presentation_feedback, update_primary_scanout_output, LuxoState, Backend},
};
use crate::{
...
impl DmabufHandler for LuxoState<UdevData> {
    ...
    fn dmabuf_imported(&mut self, _global: &DmabufGlobal, dmabuf: Dmabuf, notifier: ImportNotifier) {
            ...
            .is_ok()
        {
            ...
            let _ = notifier.successful::<LuxoState<UdevData>>();
        } else {
            notifier.failed();
        }
    }
}
delegate_dmabuf!(LuxoState<UdevData>);
...
pub fn run_udev() {
        ...
        .insert_source(notifier, move |event, &mut (), data| match event {
            ...
            SessionEvent::ActivateSession => {
                    ...
                    .map(|(handle, backend)| (*handle, backend))
                {
                    ...
                    if let Some(lease_global) = backend.leasing_global.as_mut() {
                        lease_global.resume::<LuxoState<UdevData>>();
                    }
                    data.handle
    ...
    let global = dmabuf_state
        .create_global_with_default_feedback::<LuxoState<UdevData>>(&display_handle, &default_feedback);
        ...
        .and_then(|x| x.ok())
    {
        if let Some(backend) = state.backend_data.backends.get(&primary_node) {
            ...
            if supports_syncobj_eventfd(&import_device) {
                let syncobj_state =
                    DrmSyncobjState::new::<LuxoState<UdevData>>(&display_handle, import_device);
...
impl DrmLeaseHandler for LuxoState<UdevData> {
...
delegate_drm_lease!(LuxoState<UdevData>);

impl DrmSyncobjHandler for LuxoState<UdevData> {
    fn drm_syncobj_state(&mut self) -> &mut DrmSyncobjState {
        self.backend_data.syncobj_state.as_mut().unwrap()
    }
}
smithay::delegate_drm_syncobj!(LuxoState<UdevData>);
...
impl Drop for SurfaceData {
    fn drop(&mut self) {
        if let Some(global) = self.global.take() {
            self.dh.remove_global::<LuxoState<UdevData>>(global);
        }
    }
}
...
impl LuxoState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let registration_token = self
            ...
            .insert_source(
                notifier,
                move |event, metadata, data: &mut LuxoState<_>| match event {
        ...
        self.backend_data.backends.insert(
            ...
            BackendData {
                ...
                leasing_global: DrmLeaseState::new::<LuxoState<UdevData>>(&self.display_handle, &node)
                    .inspect_err(|err| {
    ...
    fn connector_connected(&mut self, node: DrmNode, connector: connector::Info, crtc: crtc::Handle) {
        ...
        if non_desktop {
            ...
            if let Some(lease_state) = device.leasing_global.as_mut() {
                lease_state.add_connector::<LuxoState<UdevData>>(
                    connector.handle(),
                    output_name,
                    format!("{} {}", make, model),
                );
            ...
            let global = output.create_global::<LuxoState<UdevData>>(&self.display_handle);
    ...
    fn device_removed(&mut self, node: DrmNode) {
        ...
        if let Some(mut backend_data) = self.backend_data.backends.remove(&node) {
            if let Some(mut leasing_global) = backend_data.leasing_global.take() {
                leasing_global.disable_global::<LuxoState<UdevData>>();
            }
