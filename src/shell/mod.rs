...
impl<BackendData: Backend> BufferHandler for LuxoState<BackendData> {
    fn buffer_destroyed(&mut self, _buffer: &WlBuffer) {}
}

impl<BackendData: Backend> CompositorHandler for LuxoState<BackendData> {
...
impl<BackendData: Backend> WlrLayerShellHandler for LuxoState<BackendData> {
...
fn place_new_window(
    ...
    let x_range = Uniform::new(output_geometry.loc.x, max_x).expect("Failed");
    let y_range = Uniform::new(output_geometry.loc.y, max_y).expect("Failed");
